            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-header pb-0">
                        <h6>Danh sach Mat bang</h6>
                    </div>
                    <div class="ms-md-auto pe-md-3 d-flex align-items-center">
                        <form class="input-group m-0" method="post" action="/contracts?userAction=search">
                            <input id="timKiemTang" name="timKiemTang" type="text" class="form-control"
                                   placeholder="Type ID...">
                            <button id="buttonSearch" type="submit" class="input-group-text text-body"><i
                                    class="fas fa-search" aria-hidden="true"></i></button>
                        </form>
                                ...
                                <c:forEach var="matBang" items="${matBangList}">
                                    <tr>
                                        <td class="align-middle p-3 text-center">
                                            <span class="text-secondary text-xs font-weight-bold">${matBang.getId()}</span>
                                        </td>
                                        <td class="align-middle p-2 text-center">
                                            <span class="text-secondary text-xs font-weight-bold">${matBang.getDienTich()}</span>
                                        </td>
                                        <td class="align-middle p-2 text-center">
                                            <span class="text-secondary text-xs font-weight-bold">${matBang.getTrangThai().getTen()}</span>
                                        </td>
                                        <td class="align-middle p-2 text-center">
                                            <span class="text-secondary text-xs font-weight-bold">${matBang.getTang().getTen()}</span>
                                        </td>
                                        <td class="align-middle p-2 text-center">
                                            <span class="text-secondary text-xs font-weight-bold">${matBang.getLoaiMatBang().getTen()}</span>
                                        </td>
                                        <td class="align-middle p-2 text-center">
                                            <span class="text-secondary text-xs font-weight-bold">${matBang.getGiaTien()}</span>
                                        </td>
                                        <td class="align-middle p-2 text-center">
                                            <span class="text-secondary text-xs font-weight-bold">${matBang.getNgayBatDau()}</span>
                                        </td>
                                        <td class="align-middle p-2 text-center">
                                            <span class="text-secondary text-xs font-weight-bold">${matBang.getNgayKetThuc()}</span>
                                        </td>

                                        <td class="align-middle p-2 text-center">
                                            <a href="/matbang?userAction=edit&id=${matBang.getId()}"
                                               class="text-secondary font-weight-bold text-xs badge badge-sm bg-gradient-light"
                                               data-toggle="tooltip" data-original-title="Edit Employee">
                                                Sua
                                            </a>
                                            <a href="/matbang?userAction=delete&id=${matBang.getId()}"
                                               onclick="return confirm('Do you want to delete ${matBang.getId()} ?')"
                                               class="text-secondary font-weight-bold text-xs badge badge-sm bg-gradient-light"
